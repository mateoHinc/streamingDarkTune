...
function UserReviewsSection() {
  return (
    <section className="bg-primary py-16 min-h-screen text-white text-center">
            ...
            <SwiperSlide key={user.id} className="flex h-full">
              <div className="flex flex-col justify-between items-center bg-secondary shadow-lg p-6 rounded-xl w-full min-h-[350px] text-left">
                <img
                  src={user.photo}
                  alt={user.name}
                  className="shadow-md mb-4 rounded-full ring-2 ring-white ring-offset-2 w-20 sm:w-24 md:w-28 h-20 sm:h-24 md:h-28 object-cover"
                />
                <h3 className="mb-2 font-semibold text-base sm:text-lg md:text-xl">
                  {user.name}
                </h3>
